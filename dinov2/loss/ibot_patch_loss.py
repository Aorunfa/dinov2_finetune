...
try:
    ...
    def lossfunc(t, s, temp):
        return torch.sum(t * F.log_softmax(s / temp, dim=-1), dim=-1) # pt * log(ps)
...
class iBOTPatchLoss(nn.Module):
    ...
    def forward_masked(
        ...
        # loss = torch.sum(t * F.log_softmax(s / self.student_temp, dim=-1), dim=-1)
        loss = lossfunc(t, s, self.student_temp)                    # 计算logits的entropy
        ...
        loss = loss * masks_weight                                  # 每张图片内自己进行平均
        return -loss.sum() / student_masks_flat.shape[0]            # 计算图片层级的均值
